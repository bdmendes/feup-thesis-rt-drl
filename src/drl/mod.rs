...
impl Conv2d {
    pub fn new(
        ...
        p.insert(
            "kernel".to_string(),
            mem.new_push(&[out_channel, in_channel, kernel_size, kernel_size], true),
        );
        p.insert(
            "bias".to_string(),
            mem.push(
                Tensor::full(&[out_channel], 0.0, (Kind::Float, Device::Cpu)).requires_grad_(true),
            ),
        );
        Self {
            params: p,
...
impl RNN {
    pub fn new(
        ...
        p.insert(
            "Wxh".to_string(),
            mem.new_push(&[input_size, hidden_size], true),
        );
        p.insert(
            "Whh".to_string(),
            mem.new_push(&[hidden_size, hidden_size], true),
        );
        p.insert("bh".to_string(), mem.new_push(&[hidden_size], true));
        if linear_layer {
            p.insert(
                "W".to_string(),
                mem.new_push(&[hidden_size, linear_out_size], true),
            );
...
pub fn accuracy(target: &Tensor, pred: &Tensor) -> f64 {
    ...
    let accuracy: f64 = (eq.sum(Kind::Int64) / target.size()[0])
        .double_value(&[])
        .into();
    accuracy
}
...
pub fn convert_to_lagged_series(x: &Tensor, n: i64) -> Tensor {
    ...
    x.unfold(0, n, 1).view((num_samples, n)).copy()
}
...
pub fn plot_graph(x: &Tensor, y: &Tensor) {
    ...
    figure
        .axes2d()
        .points(&x_vec, &y_vec, &[Caption("Data"), Color("red")]);
...
//     let action = if random_number > epsilon {
